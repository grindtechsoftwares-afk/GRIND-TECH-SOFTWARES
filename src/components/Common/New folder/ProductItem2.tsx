...
const ProductItem = ({ item }: { item: Product }) => {
  ...
  return (
    <div className="group">
      ...
      >
        <Link href="https://www.facebook.com/profile.php?id=61562873491047"> {item.title} </Link>
      </h3>

      <span className="flex items-center gap-2 font-medium text-lg">
        <span className="text-dark">P{item.discountedPrice}</span>
        <span className="text-dark-4 line-through">P{item.price}</span>
      </span>
    </div>
  );
};
